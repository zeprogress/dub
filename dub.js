...
(function () {
    ...
    // На одном Android TV (древний WebView) настоящие https-запросы к ЛЮБОМУ
    // внешнему хосту зависают на уровне TLS-рукопожатия (проверено и на
    // Cloudflare Workers, и на Netlify — оба одинаково виснут), при этом
    // обычный http внутри локальной сети у этого же устройства работает
    // мгновенно и без сбоев (TorrServer тому пример). Раньше пробовали
    // ходить по http прямо на fish-tts-proxy.player2vr.workers.dev, но
    // *.workers.dev в современных браузерах принудительно апгрейдится на
    // https через HSTS-preload ещё до отправки запроса — получали либо
    // TLS-зависание (апгрейд сработал), либо CORS-сбой после редиректа.
    // Теперь прокси живёт на собственном хостинге (Beget) не в HSTS-preload
    // списке и отдаёт обычный http без принудительного редиректа —
    // TV может стучаться сюда так же плоско, как и на TorrServer.
    var TTS_PROXY_URL = 'http://zeprogut.beget.tech/beget_tts_proxy.php';
    ...
    var TTS_FETCH_TIMEOUT_MS = 8000;  // сколько ждать ответа от TTS-прокси на ОДНУ попытку; коротко специально — рабочие запросы отвечают за ~1.5с, а зависшие всё равно не оживут, лучше быстрее уйти на повтор (см. SYNTH_RETRIES), чем ждать долгий таймаут на каждой из нескольких попыток
